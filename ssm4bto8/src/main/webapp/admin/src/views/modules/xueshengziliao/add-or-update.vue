      ...
      <el-col :span="24">  
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.ziliaowenjian" label="资料文件" prop="ziliaowenjian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.ziliaowenjian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
	...
	this.ruleForm.ziliaoneirong = this.ruleForm.ziliaoneirong.replace(reg,'../../../ssm4bto8/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xueshengziliao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xueshengziliaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
