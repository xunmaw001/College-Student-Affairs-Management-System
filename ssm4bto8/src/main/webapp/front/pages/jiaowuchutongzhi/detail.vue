...
<script>
	export default {
		...
		methods: {
			...
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','jiaowuchutongzhi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','jiaowuchutongzhi');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('jiaowuchutongzhi', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussjiaowuchutongzhi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('jiaowuchutongzhi', this.detail);
				this.$utils.msg('审核成功');
